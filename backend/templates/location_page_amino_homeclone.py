...
def generate_location_page_html(
    ...
    # Generate county cards with icons
    county_cards = "".join(
        [
            (
                f'<a class="location-card" href="/locations/123Bots-{_slugify(county.replace(" County", ""))}-{state_abbr.lower()}">'
            ...
            for county in counties[:24]
        ]
    )
    # Generate city cards with icons
    city_cards = "".join(
        [
            (
                f'<a class="location-card" href="/locations/123Bots-{_slugify(city)}-{state_abbr.lower()}">'
    ...
    breadcrumb = (
        f'<a href="/locations/123Bots-{state_slug}">{safe_state_name}</a> <span class="sep">/</span> <span>{safe_location_name}</span>'
        if location_type != "state"
        else f"<span>{safe_location_name}</span>"
    )

    # Build the location navigation section based on location type
    if location_type == "state":
        # State pages show both counties and cities tabs
        location_nav_section = f'''
      <div class="coverage-tabs">
        <button class="coverage-tab active" onclick="switchTab('counties')">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 21h18M9 8h10M9 12h10M9 16h10M5 8v.01M5 12v.01M5 16v.01"/></svg>
          Counties
          <span class="tab-count">{county_count}</span>
        </button>
        <button class="coverage-tab" onclick="switchTab('cities')">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 21h18M5 21V7l8-4v18M13 21V3l6 3v15M9 9v.01M9 13v.01M9 17v.01M17 9v.01M17 13v.01M17 17v.01"/></svg>
          Cities
          <span class="tab-count">{city_count}</span>
        </button>
      </div>
      
      <div id="counties-tab" class="tab-content active">
        <div class="location-grid">{county_cards}</div>
        {f'<div class="view-all-container"><span class="view-all-btn">Showing {min(len(counties), 24)} of {county_count} counties</span></div>' if len(counties) > 24 else ''}
      </div>
      
      <div id="cities-tab" class="tab-content">
        <div class="location-grid">{city_cards}</div>
        {f'<div class="view-all-container"><span class="view-all-btn">Showing {min(len(cities), 24)} of {city_count} cities</span></div>' if len(cities) > 24 else ''}
      </div>
      
      <script>
        function switchTab(tab) {{
          document.querySelectorAll('.coverage-tab').forEach(btn => btn.classList.remove('active'));
          document.querySelectorAll('.tab-content').forEach(content => content.classList.remove('active'));
          
          if (tab === 'counties') {{
            document.querySelector('.coverage-tab:first-child').classList.add('active');
            document.getElementById('counties-tab').classList.add('active');
          }} else {{
            document.querySelector('.coverage-tab:last-child').classList.add('active');
...
'''
        ...
        # County pages show cities within the county (or state cities as fallback)
        location_nav_section = f'''
      <h3 style="margin-bottom:1.5rem;color:var(--purple-mid);font-size:1.25rem;font-weight:600;">Cities Near {safe_location_name}</h3>
      <div class="location-grid">{city_cards}</div>
      {f'<div class="view-all-container"><span class="view-all-btn">Showing {min(len(cities), 24)} cities</span></div>' if len(cities) > 24 else ''}
'''
    else:
...
<head>
  ...
  <meta name="keywords" content="123Bots, custom products {escape(location_name)}, sublimation printing {escape(location_name)}, custom t-shirts, mugs, tumblers" />
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    h1, h2, h3, h4 {{ font-family: 'Cinzel', serif; font-weight: 600; }}
    ...
    .hero-bg-image {{
      ...
      width: 100%;
      height: 100%;
      background-image: url('https://customer-assets.emergentagent.com/job_cart-builder-21/artifacts/dk8ihy2p_gingerkare-emporuim-and-collectibles.png');
    ...
    .hero-feature-icon svg {{ width: 22px; height: 22px; stroke: var(--gold-light); }}
    .hero-feature-text h4 {{ font-size: 14px; color: white; margin-bottom: 2px; }}
    .hero-feature-text p {{ font-size: 12px; color: rgba(255, 255, 255, 0.65); }}
    ...
    .hero-card-overlay {{
      position: absolute;
      inset: 0;
      background: linear-gradient(180deg, transparent 40%, rgba(15, 4, 25, 0.9) 100%);
    }}
    .hero-card-content {{
      position: absolute;
      bottom: 0;
    ...
    .badge-verified svg {{ width: 16px; height: 16px; stroke: #1a7f3c; }}
    ...
    .feature-card:hover {{
      ...
      box-shadow: 0 16px 40px rgba(75, 29, 119, 0.1);
      transform: translateY(-4px);
    }}
    .feature-icon {{
      width: 56px;
    ...
    .feature-card:hover .feature-icon {{
      box-shadow: 0 8px 20px rgba(214, 168, 90, 0.4);
    }}
    .feature-icon svg {{ width: 26px; height: 26px; stroke: white; }}
    .feature-card h3 {{
      font-size: 18px;
      color: var(--purple-dark);
      margin-bottom: 10px;
    ...
    .category-card {{
      background: linear-gradient(180deg, var(--bg-purple-soft), white);
      border-radius: 20px;
      padding: 32px;
      border: 1px solid var(--border-light);
      transition: all 0.3s;
    ...
    .category-card:hover {{
      border-color: var(--gold-mid);
      box-shadow: 0 12px 32px rgba(75, 29, 119, 0.12);
    }}
    .category-card h3 {{
      font-size: 20px;
      color: var(--purple-dark);
    ...
    .category-link:hover {{ gap: 10px; color: var(--gold-dark); }}
    .category-link svg {{ width: 16px; height: 16px; }}
    
    /* ===== LOCATION COVERAGE ===== */
    .coverage {{
      padding: 80px 0;
      background: linear-gradient(180deg, var(--bg-purple-soft) 0%, white 100%);
    }}
    .coverage-container {{
      max-width: 1280px;
      margin: 0 auto;
    ...
    .coverage-badge {{
      ...
      gap: 8px;
      padding: 10px 20px;
      background: rgba(75, 29, 119, 0.08);
      border: 1px solid var(--purple-mid);
      border-radius: 50px;
      margin-bottom: 20px;
    }}
    .coverage-badge svg {{ width: 18px; height: 18px; stroke: var(--purple-mid); }}
    .coverage-badge span {{
      font-size: 12px;
      font-weight: 700;
      letter-spacing: 0.1em;
    ...
    .coverage-tab svg {{ width: 20px; height: 20px; stroke: currentColor; }}
    .coverage-tab:hover {{
    ...
    .coverage-tab.active {{
      ...
      border-color: var(--purple-mid);
      color: white;
      box-shadow: 0 8px 24px rgba(75, 29, 119, 0.25);
    }}
    .coverage-tab .tab-count {{
      background: rgba(255,255,255,0.2);
      padding: 2px 10px;
      border-radius: 20px;
      font-size: 12px;
    }}
    .coverage-tab.active .tab-count {{
    ...
    .location-card:hover {{
      border-color: var(--gold-mid);
      box-shadow: 0 8px 24px rgba(75, 29, 119, 0.1);
      transform: translateY(-2px);
    }}
    ...
    .empty-state-icon svg {{ width: 28px; height: 28px; stroke: var(--purple-mid); }}
    ...
    .compliance-note strong {{ color: #4a3507; }}
    
    /* ===== TESTIMONIALS ===== */
    .testimonials {{
      padding: 100px 0;
      background: linear-gradient(180deg, var(--purple-deep) 0%, var(--purple-dark) 50%, var(--purple-mid) 100%);
      position: relative;
      overflow: hidden;
    }}
    .testimonials::before {{
    ...
    .cta {{
      padding: 100px 0;
      background: linear-gradient(180deg, white 0%, var(--bg-purple-soft) 100%);
      position: relative;
      overflow: hidden;
    }}
    .cta::before {{
      ...
      position: absolute;
      inset: 0;
      background-image: linear-gradient(rgba(75, 29, 119, 0.04) 1px, transparent 1px),
                        linear-gradient(90deg, rgba(75, 29, 119, 0.04) 1px, transparent 1px);
    ...
    .btn-purple {{
      background: linear-gradient(135deg, var(--purple-mid), var(--purple-light));
      color: white;
      box-shadow: 0 8px 24px rgba(75, 29, 119, 0.3);
    }}
    .btn-purple:hover {{
      box-shadow: 0 12px 32px rgba(75, 29, 119, 0.45);
      transform: translateY(-2px);
    }}
    .btn-outline-purple {{
      background: white;
    ...
    .footer-logo-icon svg {{ width: 20px; height: 20px; stroke: white; }}
    .footer-logo span {{
    ...
    @media (max-width: 768px) {{
      ...
      .features-grid, .categories-grid, .testimonial-grid {{ grid-template-columns: 1fr; }}
      ...
      .coverage-tab {{ padding: 12px 20px; font-size: 13px; }}
      .location-card {{ padding: 14px 16px; }}
      .location-card-icon {{ width: 38px; height: 38px; }}
      .location-card-icon svg {{ width: 18px; height: 18px; }}
    }}
...
<body>
    ...
    <div class="container">
      <div class="top-bar-inner">
        <div class="top-bar-left">
          <a href="mailto:support@123bots.com" class="top-bar-item">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
            <span>support@123bots.com</span>
          </a>
        </div>
        ...
        <div class="top-bar-right">
          ...
          <a href="/register" class="btn-register">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" y1="8" x2="19" y2="14"/><line x1="22" y1="11" x2="16" y2="11"/></svg>
            Register
          </a>
        </div>
      </div>
  ...
  <nav class="navbar">
    <div class="container">
      <div class="navbar-inner">
        <a href="/" class="logo">
          <img src="https://customer-assets.emergentagent.com/job_cart-builder-21/artifacts/z0w87i4a_gingerkare-logo-3-blue.png" alt="123Bots" style="height: 44px; width: auto;">
        </a>
        <div class="nav-links">
  ...
  <section class="hero">
    ...
    <video class="hero-video" autoplay muted loop playsinline>
      <source src="/videos/butterfly_alpha.webm" type="video/webm">
    </video>
      ...
      <div class="hero-content">
        <div class="hero-left">
          <div class="hero-badge">
            ...
            <span>{type_label} Coverage - {location_label}</span>
          </div>
        ...
        <div class="hero-right">
          <div class="hero-card hero-card-clean" id="hero-card-module">
            <img src="" alt="Custom products" class="hero-card-img" id="hero-card-image"/>
            <div class="hero-card-overlay"></div>
            <div class="hero-card-content">
              <p class="hero-card-label" id="hero-card-title">CUSTOM EMPORIUM</p>
              <h3 class="hero-card-title-main" id="hero-card-subtitle">Unique & Personalized</h3>
              <p class="hero-card-subtitle" id="hero-card-description">Made with care, just for you</p>
            </div>
          </div>
        </div>
      </div>
  ...
  <!-- Features Section -->
  <section class="features collections-section">
    <div class="container">
  ...
  <script>
    // Fetch hero settings and populate the hero card module
    (function() {{
      fetch('/api/settings/hero-display')
        .then(response => response.json())
        .then(data => {{
          const heroImage = document.getElementById('hero-card-image');
          const heroTitle = document.getElementById('hero-card-title');
          const heroSubtitle = document.getElementById('hero-card-subtitle');
          const heroDescription = document.getElementById('hero-card-description');
          
          if (data.hero_card_image_url && heroImage) {{
            heroImage.src = data.hero_card_image_url;
          }}
          if (data.hero_card_title && heroTitle) {{
            heroTitle.textContent = data.hero_card_title;
          }}
          if (data.hero_card_subtitle && heroSubtitle) {{
            heroSubtitle.textContent = data.hero_card_subtitle;
          }}
          if (data.hero_card_description && heroDescription) {{
            heroDescription.textContent = data.hero_card_description;
          }}
        }})
        .catch(err => console.log('Hero settings not loaded:', err));
    }})();
  </script>
</body>
</html>
