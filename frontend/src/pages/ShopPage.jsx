...
const ShopPage = () => {
  ...
  useEffect(() => {
    ...
    setSeoMetadata({
      title: `${categoryText}Shop | 123 Bots`,
      description: `Browse our collection of ${selectedCategory !== 'All' ? selectedCategory.toLowerCase() : 'commercial cleaning robots and accessories'}. PUDU robots, floor scrubbers, and more at 123Bots!`,
      keywords: `${selectedCategory !== 'All' ? selectedCategory + ', ' : ''}commercial robots, cleaning robots, PUDU, floor scrubbers, 123Bots shop`,
      canonicalPath: selectedCategory !== 'All' ? `/shop?category=${selectedCategory.toLowerCase().replace(/\s+/g, '-')}` : '/shop',
      ogType: 'website',
      jsonLd: {
        '@context': 'https://schema.org',
        '@type': 'CollectionPage',
        name: `123 Bots Shop${selectedCategory !== 'All' ? ` - ${selectedCategory}` : ''}`,
        description: 'Browse commercial cleaning robots and accessories',
      ...
      try {
        ...
        const transformedProducts = productsData.map((p) => {
          ...
          return {
            ...
            image: p.image || '/images/robots/pudu-cc1-pro.png',
            category: p.category,
  ...
  return (
    <div className="min-h-screen bg-bots-dark pt-28 pb-32" data-testid="shop-page">
      ...
      {/* Hero banner */}
      <section className="relative py-16 mb-8 bg-gradient-to-b from-bots-surface to-bots-dark border-b border-gray-800">
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8 text-center">
          ...
          >
            ...
            <p className="text-gray-400 text-lg max-w-2xl mx-auto">
              Browse our collection of AI-powered commercial cleaning robots and accessories. Transform your floor cleaning operations.
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className={`flex gap-8 ${left_menu_enabled ? '' : ''}`}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <motion.div
              ...
              className="mb-8"
            >
              <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between p-4 bg-bots-surface border border-gray-700 rounded-2xl">
                {/* Search */}
                <div className="relative flex-1 max-w-md">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3 bg-bots-dark border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all"
                ...
                <div className="flex flex-wrap gap-3 items-center">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="appearance-none px-4 py-3 pr-10 bg-bots-dark border border-gray-700 rounded-xl text-white focus:outline-none focus:border-blue-500 cursor-pointer"
                      data-testid="category-filter"
                    >
                      {categories.map(cat => (
                        <option key={cat} value={cat} className="bg-bots-dark">{cat}</option>
                      ))}
                    </select>
                  ...
                  <div className="relative">
                    ...
                    >
                      {sortOptions.map(opt => (
                        <option key={opt.value} value={opt.value} className="bg-bots-dark">{opt.label}</option>
                      ))}
                    </select>
                    <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 pointer-events-none" />
                  </div>
                  ...
                  <div className="flex items-center gap-1 p-1 bg-bots-dark border border-gray-700 rounded-xl">
                    <button
                      ...
                      className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-blue-600 text-white' : 'text-gray-500 hover:text-white'}`}
                    ...
                    <button
                      ...
                      className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-blue-600 text-white' : 'text-gray-500 hover:text-white'}`}
            ...
            <div className="flex items-center justify-between mb-6">
              <p className="text-gray-400 font-mono text-sm">
                Showing <span className="text-blue-400 font-semibold">{filteredProducts.length}</span> {filteredProducts.length === 1 ? 'product' : 'products'}
              </p>
              {selectedCategory !== 'All' && (
                <button
                  onClick={() => setSelectedCategory('All')}
                  className="flex items-center gap-2 px-3 py-1 bg-blue-500/10 border border-blue-500/30 rounded-full text-blue-400 text-sm hover:bg-blue-500/20 transition-all"
                >
                  {selectedCategory}
                  <X className="w-3 h-3" />
                </button>
            ...
            {loading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {[...Array(8)].map((_, i) => (
                  <div key={i} className="aspect-[3/4] bg-bots-surface border border-gray-700 rounded-2xl animate-pulse" />
                ))}
              </div>
              ...
              >
                <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-bots-surface border border-gray-700 flex items-center justify-center">
                ...
                <button
                  ...
                  className="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-full transition-all font-semibold"
                >
                  Clear Filters
                </button>
              </motion.div>
